...
void loop() {
  char command;  // This is null character chosen to represent empty state
  ...
  if (command == 'U') {
    ...
    roboclaw.ForwardM1(address, jSpeed);
    roboclaw.ForwardM2(address, jSpeed);
  }
  if (command == 'L') {
    Serial.println("Left");
    roboclaw.ForwardM1(address, jTurn);
    roboclaw.BackwardM2(address, jTurn);
  }
  if (command == 'R') {
    Serial.println("Right");
    roboclaw.BackwardM1(address, jTurn);
    roboclaw.ForwardM2(address, jTurn);
  }
  command = ' ';
  delay(5);
}
